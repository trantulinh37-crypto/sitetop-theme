...
add_action( 'wp_footer', function() {
    ...
    if ( $telegram ) {
        ...
        $items[] = [
            ...
            'label' => 'Telegram',
            'svg'   => '<svg width="20" height="20" viewBox="0 0 48 48" fill="white"><path d="M5.83 23.616c12.57-5.48 20.94-9.1 25.12-10.84 11.97-4.98 14.45-5.85 16.08-5.87.36-.01 1.16.08 1.67.5.44.35.56.82.62 1.15.06.33.13 1.08.07 1.67-.66 6.95-3.54 23.82-5 31.6-.62 3.3-1.83 4.4-3.01 4.51-2.56.24-4.5-1.69-6.98-3.32-3.88-2.55-6.07-4.13-9.83-6.62-4.35-2.88-.77-5.15 1.33-7.33.37-.39 6.77-6.21 6.9-6.74.02-.07.03-.32-.12-.45-.15-.13-.37-.09-.53-.05-.23.05-3.84 2.44-10.84 7.17-1.03.7-1.96 1.05-2.78 1.03-.92-.02-2.68-.52-3.99-.94-1.6-.52-2.88-.8-2.77-1.69.06-.46.7-.94 1.93-1.42z"/></svg>',
            'color' => '#0088cc',
        ];
    }
    if ( $signal ) {
        $signal_url = ( strpos( $signal, 'http' ) === 0 || strpos( $signal, 'signal.me' ) === 0 )
            ? ( strpos( $signal, 'http' ) === 0 ? $signal : 'https://' . $signal )
            : 'https://signal.me/#p/' . urlencode( $signal );
        $items[] = [
            'url'   => $signal_url,
            'label' => 'Signal',
            'svg'   => '<svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15l-4-4 1.41-1.41L11 14.17l6.59-6.59L19 9l-8 8z"/></svg>',
            'color' => '#3a76f0',
        ];
    }
    if ( $zalo ) {
