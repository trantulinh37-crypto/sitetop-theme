...
if(isset($_POST['campaign_action']) && wp_verify_nonce($_POST['_wpnonce'],'sitetop_campaign_action')){
    ...
    if($action === 'approve'){
        ...
        // Camp Direct được khai URL gọn "weba.com" (chủ site chốt 21/09/2026); loại khác giữ nguyên.
        $dest = sitetop_sanitize_destination_urls($dest_in, $task_type === 'traffic_direct');
        $target_url = $dest['error'] ? '' : $dest['urls'][0];
        $title = sanitize_text_field($_POST['title'] ?? '');
        /* Bắt gõ tay (ON/OFF từng CAMP), mặc định OFF. Search: gõ tay TỪ KHOÁ vào Google.
           Direct (thêm 21/09/2026): gõ tay URL ĐÍCH vào trình duyệt — camp Direct không có
           từ khoá nào, xem $url_nocopy trong page-unlock.php. Loại khác không có nút này. */
        $kw_bat_go_tay = (in_array($task_type, array('keyword_search','traffic_direct'), true) && !empty($_POST['kw_bat_go_tay'])) ? 1 : 0;
...
<div><label <?php echo $lbl; ?>>Vị trí trên Google</label><select name="serp_page" <?php echo $inp; ?>><?php for($vp=1;$vp<=10;$vp++) echo '<option value="'.$vp.'">Trang '.$vp.'</option>'; ?></select></div>
    ...
    function admUpdatePrice(){
        ...
        admApplyGoTay(t,'admCreateGoTayWrap','adm_kw_go_tay','adm_go_tay_ten','adm_go_tay_mo_ta');
        admApplyDestType('admDestList', t==='traffic_direct');
        admUpdateEstimate();
    }
...
<div style="background:#fff;border-radius:12px;width:100%;max-width:620px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2);margin:auto">
    ...
    <form id="admEditCampForm" style="padding:20px" enctype="multipart/form-data">
        ...
        <div id="admEditGoTayWrap" style="margin-bottom:12px"><style>.adm-sw{display:flex;align-items:flex-start;gap:9px;margin-top:8px;cursor:pointer;user-select:none;position:relative}.adm-sw input{position:absolute;opacity:0;width:0;height:0;margin:0}.adm-sw-track{position:relative;flex:none;width:38px;height:21px;margin-top:1px;background:#c3c4c7;border-radius:999px;transition:background .15s}.adm-sw-track:after{content:"";position:absolute;top:2px;left:2px;width:17px;height:17px;background:#fff;border-radius:50%;box-shadow:0 1px 2px rgba(0,0,0,.25);transition:transform .15s}.adm-sw input:checked+.adm-sw-track{background:#2271b1}.adm-sw input:checked+.adm-sw-track:after{transform:translateX(17px)}.adm-sw input:focus-visible+.adm-sw-track{outline:2px solid #2271b1;outline-offset:2px}.adm-sw-txt{font-size:12px;line-height:1.45;color:#50575e}.adm-sw-txt b{color:#1d2327}.adm-sw-on,.adm-sw-off{min-width:28px;margin-left:5px;padding:0 5px;border-radius:3px;font-size:10px;font-weight:700;text-align:center}.adm-sw-on{display:none;background:#e7f3ff;color:#2271b1}.adm-sw-off{display:inline-block;background:#f0f0f1;color:#646970}.adm-sw input:checked~.adm-sw-txt .adm-sw-on{display:inline-block}.adm-sw input:checked~.adm-sw-txt .adm-sw-off{display:none}</style><label class="adm-sw"><input type="checkbox" value="1" id="admEditKwGoTay"><span class="adm-sw-track"></span><span class="adm-sw-txt"><b id="admEditGoTayTen">Bắt gõ tay keyword</b><span class="adm-sw-on">ON</span><span class="adm-sw-off">OFF</span><br><span id="admEditGoTayMoTa">ON: mọi từ khoá của CAMP này đều phải gõ tay, chặn copy. OFF: chỉ từ khoá ≤ 11 ký tự tự bắt gõ tay (như hiện nay).</span></span></label></div>
...
document.getElementById('admEditCampForm').addEventListener('submit', function(e) {
    ...
    if (_admEditTaskType === 'keyword_search' || _admEditTaskType === 'traffic_direct') fd.append('kw_bat_go_tay', document.getElementById('admEditKwGoTay').checked ? '1' : '0');
...
/* Nút "Bắt gõ tay" dùng CHUNG cột kw_bat_go_tay cho hai loại camp, chỉ khác THỨ bị chặn
   copy: Search chặn từ khoá (user gõ vào Google), Direct chặn URL đích (user gõ vào trình
   duyệt — camp Direct không có từ khoá nào). Loại khác: ẩn nút và tắt cờ. */
var ADM_GO_TAY_TXT={
    keyword_search:['Bắt gõ tay keyword','ON: mọi từ khoá của CAMP này đều phải gõ tay, chặn copy. OFF: chỉ từ khoá ≤ 11 ký tự tự bắt gõ tay (như hiện nay).'],
    traffic_direct:['Bắt gõ tay URL đích','ON: trang nhiệm vụ bỏ nút Copy và chặn bôi đen URL — user phải tự gõ URL vào trình duyệt. OFF: giữ nút Copy như hiện nay.']
};
function admApplyGoTay(t, idWrap, idChk, idTen, idMoTa){
    var w=document.getElementById(idWrap); if(!w) return;
    var txt=ADM_GO_TAY_TXT[t];
    if(!txt){ w.style.display='none'; var c0=document.getElementById(idChk); if(c0)c0.checked=false; return; }
    w.style.display='';
    var ten=document.getElementById(idTen), mo=document.getElementById(idMoTa);
    if(ten) ten.textContent=txt[0];
    if(mo) mo.textContent=txt[1];
}
/* Camp Direct: ô URL để type=text, nếu không trình duyệt tự chặn dạng gọn "weba.com"
   (máy chủ tự thêm https://, xem sitetop_sanitize_destination_urls). Loại khác giữ type=url. */
function admDestLaDirect(listId){
    if(listId==='admEditDestList') return _admEditTaskType==='traffic_direct';
    var sel=document.getElementById('adm_task_type');
    return !!(sel && sel.value==='traffic_direct');
}
function admSetDestInput(inp, laDirect){
    inp.type = laDirect ? 'text' : 'url';
    inp.placeholder = laDirect ? 'weba.com hoặc https://weba.com' : 'https://...';
}
function admApplyDestType(listId, laDirect){
    var list=document.getElementById(listId); if(!list) return;
    var os=list.querySelectorAll('input[name="destination_urls[]"]');
    for(var i=0;i<os.length;i++) admSetDestInput(os[i], laDirect);
}
function admAddDest(value, listId){
    ...
    inp.name='destination_urls[]'; admSetDestInput(inp, admDestLaDirect(listId));
