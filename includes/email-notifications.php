...
function linkngon_email_wrap( $title, $content ) {
    $site_name = get_bloginfo( 'name' );
    return '
...
function linkngon_send_deposit_email( $deposit_id ) {
    if ( ! linkngon_get_option( 'email_deposit_pending', 1 ) ) return;
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Số tiền</td><td style="padding:8px 0;font-weight:600;color:#059669">' . linkngon_format_money($dep->amount) . '</td></tr>';
    ...
    wp_mail($admin_email, $subject, linkngon_email_wrap('Yêu cầu nạp tiền mới', $content), array('Content-Type: text/html; charset=UTF-8'));
}

/**
 * Email KH khi nạp tiền được duyệt
 */
function linkngon_send_deposit_approved_email( $deposit_id ) {
    if ( ! linkngon_get_option( 'email_deposit_approved', 1 ) ) return;
    ...
    $subject = '[LinkNgon] Nạp tiền thành công - ' . linkngon_format_money($dep->amount);
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Số tiền nạp</td><td style="padding:8px 0;font-weight:600">' . linkngon_format_money($dep->amount) . '</td></tr>';
    if ( floatval($dep->bonus_amount) > 0 ) {
        $content .= '<tr><td style="padding:8px 0;color:#64748b">Bonus</td><td style="padding:8px 0;font-weight:600;color:#059669">+' . linkngon_format_money($dep->bonus_amount) . '</td></tr>';
    }
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Tổng cộng</td><td style="padding:8px 0;font-weight:700;color:#059669">' . linkngon_format_money($total) . '</td></tr>';
    $content .= '</table>';
    $content .= '<p style="font-size:13px;color:#64748b">Số dư đã được cộng vào tài khoản của bạn. Bạn có thể tạo chiến dịch ngay.</p>';

    wp_mail($dep->user_email, $subject, linkngon_email_wrap('Nạp tiền thành công', $content), array('Content-Type: text/html; charset=UTF-8'));
}

/**
 * Email KH khi nạp tiền bị từ chối
 */
function linkngon_send_deposit_rejected_email( $deposit_id ) {
    if ( ! linkngon_get_option( 'email_deposit_rejected', 1 ) ) return;
    ...
    $subject = '[LinkNgon] Yêu cầu nạp tiền bị từ chối';
    $content = '<p style="color:#475569;line-height:1.6">Xin chào <strong>' . esc_html($dep->display_name) . '</strong>,</p>';
    $content .= '<p style="color:#475569;line-height:1.6">Rất tiếc, yêu cầu nạp tiền <strong>' . linkngon_format_money($dep->amount) . '</strong> của bạn đã bị từ chối.</p>';
    $content .= '<p style="color:#475569;line-height:1.6">Nếu bạn cho rằng đây là nhầm lẫn, vui lòng liên hệ hỗ trợ.</p>';

    wp_mail($dep->user_email, $subject, linkngon_email_wrap('Yêu cầu nạp tiền bị từ chối', $content), array('Content-Type: text/html; charset=UTF-8'));
}
 ...
 */
function linkngon_send_withdrawal_pending_email( $withdrawal_id ) {
    if ( ! linkngon_get_option( 'email_withdrawal_pending', 1 ) ) return;
    ...
    $admin_email = get_option('admin_email');
    $subject = '[LinkNgon] Yêu cầu rút tiền mới - ' . linkngon_format_money($w->amount);
    $content = '<table style="width:100%;border-collapse:collapse;font-size:14px">';
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Số tiền</td><td style="padding:8px 0;font-weight:700;color:#dc2626">' . linkngon_format_money($w->amount) . '</td></tr>';
    ...
    wp_mail($admin_email, $subject, linkngon_email_wrap('Yêu cầu rút tiền mới', $content), array('Content-Type: text/html; charset=UTF-8'));
}

/**
 * Email user khi withdrawal thay đổi trạng thái
 */
function linkngon_send_withdrawal_status_email( $withdrawal_id, $new_status ) {
    $setting_map = array(
    ...
    if ( ! linkngon_get_option( $setting_map[$new_status], 1 ) ) return;
    ...
    $subject = '[LinkNgon] ' . $info['title'] . ' - ' . linkngon_format_money($w->amount);
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Số tiền</td><td style="padding:8px 0;font-weight:700">' . linkngon_format_money($w->amount) . '</td></tr>';
    ...
    wp_mail($w->user_email, $subject, linkngon_email_wrap($info['title'], $content), array('Content-Type: text/html; charset=UTF-8'));
}
 ...
 */
function linkngon_send_report_error_email( $session_id, $error_type, $error_message ) {
    if ( ! linkngon_get_option( 'email_report_error', 1 ) ) return;
    ...
    $admin_email = get_option('admin_email');
    $subject = '[LinkNgon] User báo lỗi mã - ' . esc_html($error_type);
    $content = '<table style="width:100%;border-collapse:collapse;font-size:14px">';
    if ( $visit ) {
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Thời gian</td><td style="padding:8px 0">' . linkngon_current_time() . '</td></tr>';
    $content .= '</table>';

    wp_mail($admin_email, $subject, linkngon_email_wrap('User báo lỗi mã xác minh', $content), array('Content-Type: text/html; charset=UTF-8'));
}
 ...
 */
function linkngon_send_new_campaign_email( $campaign_id ) {
    if ( ! linkngon_get_option( 'email_campaign_new', 1 ) ) return;
    ...
    $admin_email = get_option('admin_email');
    $subject = '[LinkNgon] Chiến dịch mới cần duyệt - ' . esc_html($c->title);
    $content = '<table style="width:100%;border-collapse:collapse;font-size:14px">';
    ...
    $content .= '<tr><td style="padding:8px 0;color:#64748b">Giá/view</td><td style="padding:8px 0;font-weight:600">' . linkngon_format_money($c->price_per_view) . '</td></tr>';
    ...
    wp_mail($admin_email, $subject, linkngon_email_wrap('Chiến dịch mới cần duyệt', $content), array('Content-Type: text/html; charset=UTF-8'));
}
