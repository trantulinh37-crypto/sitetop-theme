 ...
 * `traffictop_report_autopause_threshold`). Đếm theo IP DUY NHẤT để chống 1 người spam nút
...
function traffictop_report_autopause_enabled() {
	return (int) traffictop_get_option( 'report_autopause_enabled', 1 ) === 1;
}

/** Ngưỡng số IP khác nhau/giờ để tạm dừng (mặc định 5). */
function traffictop_report_autopause_threshold() {
	$n = (int) traffictop_get_option( 'report_autopause_threshold', 5 );
	return $n > 0 ? $n : 5;
}

/** user_id tài khoản liên kết cầu nối (bridged campaigns thuộc tài khoản này). 0 nếu không có. */
function traffictop_report_fed_customer() {
...
function traffictop_report_is_bridged( $campaign ) {
	$fed     = traffictop_report_fed_customer();
...
function traffictop_report_autopause_tele( $campaign, $distinct, $bridged ) {
	...
	traffictop_telegram_notify_admin( $title, $rows );
}
 ...
 */
function traffictop_report_autopause_check( $campaign_id, $ip ) {
	$campaign_id = (int) $campaign_id;
	$ip          = (string) $ip;
	if ( ! $campaign_id || '' === $ip || ! traffictop_report_autopause_enabled() ) {
		return;
	}

	$window = HOUR_IN_SECONDS;
	$now    = time();
	$key    = 'traffictop_reperr_' . $campaign_id;
	...
	$distinct = count( $map );
	if ( $distinct < traffictop_report_autopause_threshold() ) {
		return;
	}

	// Chống spam Telegram + pause lặp: chỉ hành động 1 lần / giờ / camp.
	$cooldown = 'traffictop_repalert_' . $campaign_id;
...
function traffictop_report_autopause_on_report( $session_id, $ip ) {
	$session_id = (string) $session_id;
	if ( '' === $session_id || ! $ip || ! traffictop_report_autopause_enabled() ) {
		return;
	}
	global $wpdb;
	$p   = $wpdb->prefix . 'traffictop_';
	...
	if ( $cid > 0 ) {
		traffictop_report_autopause_check( $cid, $ip );
	}
}
