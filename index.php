...
.ln-hero h1{font-family:'DM Serif Display',serif;font-size:44px;color:#fff;margin-bottom:12px;line-height:1.2}
...
.ln-shorten-form input{flex:1;padding:16px 20px;background:rgba(255,255,255,.95);border:none;border-radius:12px;font-family:'Be Vietnam Pro',sans-serif;font-size:15px;color:#2C2C3A;outline:none}
.ln-shorten-form input::placeholder{color:#9CA3AF}
.ln-shorten-form button{padding:16px 32px;background:#E8A838;color:#083838;border:none;border-radius:12px;font-family:'Be Vietnam Pro',sans-serif;font-size:15px;font-weight:700;cursor:pointer;transition:all .25s;white-space:nowrap;margin-left:6px}
...
.ln-result-url button{padding:14px 20px;background:#059669;color:#fff;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-family:'Be Vietnam Pro',sans-serif;transition:all .2s}
...
.ln-counter-value{font-family:'DM Serif Display',serif;font-size:32px;color:#E8A838}
...
.ln-section-title h2{font-family:'DM Serif Display',serif;font-size:34px;color:#083838;margin-bottom:8px}
...
.ln-step-num{width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#0D4F4F,#1A7A7A);color:#fff;display:flex;align-items:center;justify-content:center;font-family:'DM Serif Display',serif;font-size:24px;margin:0 auto 16px;box-shadow:0 4px 14px rgba(13,79,79,.2)}
.ln-step h3{font-family:'DM Serif Display',serif;font-size:18px;color:#083838;margin-bottom:6px}
.ln-step p{font-size:13px;color:#6B7280;line-height:1.6}
.ln-step-arrow{position:absolute;right:-20px;top:28px;font-size:20px;color:#E5E2DB}
...
.ln-rate-flag{font-size:32px;margin-bottom:8px}
.ln-rate-country{font-weight:600;color:#083838;font-size:15px;margin-bottom:4px}
.ln-rate-price{font-family:'DM Serif Display',serif;font-size:28px;color:#0D4F4F}
...
.ln-feat-icon{font-size:36px;margin-bottom:12px}
.ln-feat h3{font-family:'DM Serif Display',serif;font-size:18px;color:#083838;margin-bottom:6px}
...
.ln-referral h2{font-family:'DM Serif Display',serif;font-size:32px;color:#fff;margin-bottom:10px}
...
.ln-referral-highlight{font-family:'DM Serif Display',serif;font-size:48px;color:#E8A838;margin-bottom:8px}
.ln-cta-btn{display:inline-flex;padding:14px 36px;background:#E8A838;color:#083838;border-radius:12px;font-weight:700;font-size:15px;text-decoration:none;transition:all .25s}
...
<section class="ln-features">
    ...
    <div class="ln-feat-grid">
        <div class="ln-feat"><div class="ln-feat-icon">📊</div><h3>Thống kê chi tiết</h3><p>Theo dõi clicks, quốc gia, thiết bị, referer theo thời gian thực.</p></div>
        <div class="ln-feat"><div class="ln-feat-icon">⚡</div><h3>API rút gọn</h3><p>API nhanh để tích hợp vào website, app hoặc script tự động.</p></div>
        <div class="ln-feat"><div class="ln-feat-icon">🔒</div><h3>Link an toàn</h3><p>Chống spam, malware. Link của bạn luôn hoạt động ổn định.</p></div>
        <div class="ln-feat"><div class="ln-feat-icon">💸</div><h3>Rút tiền nhanh</h3><p>Rút tối thiểu <?php echo linkngon_format_money( linkngon_get_option( 'min_withdrawal', 50000 ) ); ?>. Chuyển khoản ngân hàng hoặc MoMo.</p></div>
        <div class="ln-feat"><div class="ln-feat-icon">👥</div><h3>Referral 20%</h3><p>Giới thiệu bạn bè và nhận 20% thu nhập của họ — vĩnh viễn!</p></div>
        <div class="ln-feat"><div class="ln-feat-icon">🌐</div><h3>Đa nền tảng</h3><p>Hoạt động trên mọi thiết bị. Link rút gọn tương thích mọi nơi.</p></div>
    </div>
</section>
...
<section class="ln-referral">
    ...
    <?php if ( $is_logged ) : ?>
        <a href="<?php echo get_permalink( get_page_by_path('dashboard') ); ?>" class="ln-cta-btn">📊 Vào Dashboard</a>
    <?php else : ?>
        <a href="<?php echo get_permalink( get_page_by_path('login') ); ?>?action=register" class="ln-cta-btn">🚀 Đăng ký ngay — Miễn phí</a>
    <?php endif; ?>
</section>
