        ...
        <div class="card">
            <div class="card-header">
                ...
                <div style="display:flex;gap:8px;">
                    ...
                    <button class="btn btn-sm btn-outline" onclick="loadCampaigns()">🔄</button>
                </div>
            </div>
    ...
    ajax('linkngon_admin_get_campaigns', { status: status }, function(r) {
        ...
        tbody.innerHTML = r.data.campaigns.map(function(c) {
            ...
            return '<tr>' +
                ...
                '<td><button class="btn btn-sm btn-outline" onclick="toggleCampaign(' + c.id + ',\'' + c.status + '\')">' + (c.status === 'active' ? '⏸️' : '▶️') + '</button></td>' +
                '</tr>';
        }).join('');
    });
}
