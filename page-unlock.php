...
<?php if ( (int) sitetop_get_option( 'block_incognito', 1 ) === 1 ) : ?>
<!-- Chặn tab ẩn danh. Ẩn sẵn, chỉ JS bật lên khi nhận ra. -->
<div id="stAnDanh" style="display:none;position:fixed;inset:0;z-index:2147483646;background:#141C33;color:#fff;
     font-family:system-ui,-apple-system,'Segoe UI',sans-serif;align-items:center;justify-content:center;padding:24px">
    <div style="max-width:420px;width:100%;text-align:center">
        <div style="width:60px;height:60px;margin:0 auto 18px;border-radius:50%;background:rgba(226,84,64,.15);
             display:flex;align-items:center;justify-content:center">
            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#E25440" stroke-width="2.2" stroke-linecap="round">
                <path d="M12 8v5M12 17h.01"/><circle cx="12" cy="12" r="9"/></svg>
        </div>
        <h2 style="margin:0 0 10px;font-size:21px;font-weight:800;letter-spacing:-.01em">Vui lòng tắt ẩn danh</h2>
        <p style="margin:0 0 18px;font-size:14.5px;line-height:1.65;color:#B9C4DB">
            Nhiệm vụ không chạy được trong <b style="color:#fff">tab ẩn danh</b>.
            Hãy mở lại link này bằng <b style="color:#fff">cửa sổ thường</b> rồi làm lại.
        </p>
        <div style="text-align:left;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);
             border-radius:8px;padding:13px 15px;margin-bottom:18px;font-size:13.5px;line-height:1.9;color:#B9C4DB">
            <div>1. Đóng tab ẩn danh này</div>
            <div>2. Mở trình duyệt như bình thường</div>
            <div>3. Dán lại link nhiệm vụ</div>
        </div>
        <button type="button" onclick="location.reload()"
                style="width:100%;height:46px;border:0;border-radius:8px;background:#3B6FB6;color:#fff;
                       font-size:15px;font-weight:700;cursor:pointer;font-family:inherit">
            Tôi đã tắt ẩn danh, thử lại
        </button>
    </div>
</div>
<script>
/* Nhận diện chế độ ẩn danh.
   ------------------------------------------------------------------
   KHÔNG có API chuẩn nào cho việc này — mỗi trình duyệt phải suy ra từ dấu vết
   riêng, và dấu vết đó có thể sai ở máy cấu hình lạ. Nên đặt ngưỡng chặt và
   chỉ chặn khi CHẮC: thà bỏ lọt vài ca còn hơn chặn oan người làm thật, vì
   chặn oan là user mất tiền công và admin mất uy tín.
   Admin tắt được ở Cài đặt → Chặn trình duyệt ẩn danh nếu chặn nhầm. */
(function(){
    function hien(){
        var o = document.getElementById('stAnDanh');
        if (!o) return;
        o.style.display = 'flex';
        document.documentElement.style.overflow = 'hidden';

        /* Che giao diện KHÔNG đủ: trang vẫn hỏi mã mỗi 2 giây và tự điền mã vào ô,
           nên chỉ cần xoá lớp phủ trong DevTools là lấy được mã. Dừng hẳn mọi hẹn
           giờ đang chạy để luồng nhiệm vụ đứng lại thật sự. */
        try {
            var maxId = setTimeout(function(){}, 0);
            for (var i = 1; i <= maxId; i++) { clearTimeout(i); clearInterval(i); }
        } catch(e){}
    }
    function do_() {
        var ua = navigator.userAgent || '';

        // Firefox riêng tư: service worker bị tắt hẳn, đây là dấu hiệu chắc chắn.
        if (/Firefox/i.test(ua) && !/Seamonkey/i.test(ua)) {
            try { if (!('serviceWorker' in navigator)) return hien(); } catch(e){}
            return;
        }
        ...
        if (laChromium && navigator.storage && navigator.storage.estimate) {
            navigator.storage.estimate().then(function(est){
                if (est && typeof est.quota === 'number' && est.quota > 0 && est.quota < 150*1024*1024) hien();
            }).catch(function(){});
        }
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', do_);
    else do_();
})();
</script>
<?php endif; ?>
